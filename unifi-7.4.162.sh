...
# Version               | 5.4.0
# Application version   | 7.4.162-3116043f9f
...
abort() {
  ...
  if dpkg -l tar 2> /dev/null | grep -iq "^ii\\|^hi"; then
    tar -cvf /tmp/eus_support.tar.gz "/tmp/EUS" "${eus_dir}" "/usr/lib/unifi/logs" &> /dev/null && support_file="/tmp/eus_support.tar.gz"
  elif dpkg -l zip 2> /dev/null | grep -iq "^ii\\|^hi"; then
    zip -r /tmp/eus_support.zip "/tmp/EUS/*" "${eus_dir}/*" "/usr/lib/unifi/logs/*" &> /dev/null && support_file="/tmp/eus_support.zip"
  fi
...
get_distro() {
  ...
  elif [[ "${os_codename}" =~ ^(jammy|vanessa|vera)$ ]]; then repo_codename=jammy; os_codename=jammy
...
get_repo() {
  if [[ -z "$(command -v lsb_release)" ]]; then
    if [[ -f "/etc/os-release" ]]; then
      release_id="$(grep -i "^ID=" /etc/os-release | sed "s/ID=//g" | tr '[:upper:]' '[:lower:]')"
    fi
  else
  ...
  if dpkg -l curl 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
    if [[ "${release_id}" == "ubuntu" ]]; then
      if curl -s http://old-releases.ubuntu.com/ubuntu/dists/ | grep -iq "${os_codename}" 2> /dev/null; then
        archived_repo=true
      fi
    elif [[ "${release_id}" == "debian" ]]; then
      if curl -s http://archive.debian.org/debian/dists/ | grep -iq "${os_codename}" 2> /dev/null; then
        archived_repo=true
      fi
    fi
  fi
...
if ! [[ "${os_codename}" =~ (precise|maya|trusty|qiana|rebecca|rafaela|rosa|xenial|sarah|serena|sonya|sylvia|bionic|tara|tessa|tina|tricia|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|jessie|stretch|continuum|buster|bullseye|bookworm|trixie|forky) ]]; then
  clear
  header_red
...
armhf_recommendation() {
  ...
  if [[ "${print_architecture}" == 'armhf' && "${is_cloudkey}" == "false" ]]; then
    ...
    if [[ "${os_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
...
apt_get_install_package() {
  ...
  if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg6::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${required_package}" &>> "${eus_dir}/logs/apt.log"; then
    ...
    if [[ "${required_package}" == "openjdk-11-jre-headless" ]] && [[ "${repo_codename}" =~ (stretch|continuum) ]]; then
      if grep -iq "openjdk-11-jre-headless.*Depends.*libjpeg8" "${eus_dir}/logs/apt.log"; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.debian.org/debian stretch-backports main") -eq 0 ]]; then
          echo "deb http://archive.debian.org/debian stretch-backports main" >> /etc/apt/sources.list.d/glennr-install-script.list || abort
          hide_apt_update=true
          run_apt_get_update
        fi
...
if ! dpkg -l sudo 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install sudo &>> "${eus_dir}/logs/required.log"; then
    ...
    if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename} main") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename} main" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      else
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu ${repo_codename} main") -eq 0 ]]; then
          echo -e "deb http://archive.ubuntu.com/ubuntu ${repo_codename} main" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      fi
    elif [[ "${repo_codename}" =~ (jessie|stretch|buster|bullseye|bookworm|trixie|forky) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://ftp.debian.org/debian ${repo_codename} main") -eq 0 ]]; then
          echo -e "deb http://ftp.debian.org/debian ${repo_codename} main" >> /etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      else
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.debian.org/debian ${repo_codename} main") -eq 0 ]]; then
          echo -e "deb http://archive.debian.org/debian ${repo_codename} main" >> /etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      fi
    fi
...
if ! dpkg -l jq 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install jq &>> "${eus_dir}/logs/required.log"; then
    ...
    if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename}-security main universe") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename}-security main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      else
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu ${repo_codename}-security main universe") -eq 0 ]]; then
          echo -e "deb http://archive.ubuntu.com/ubuntu ${repo_codename}-security main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      fi
    elif [[ "${repo_codename}" =~ (jessie|stretch|buster|bullseye|bookworm|trixie|forky) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
...
if ! dpkg -l lsb-release 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install lsb-release &>> "${eus_dir}/logs/required.log"; then
    ...
    if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename} main universe") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename} main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      else
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu ${repo_codename} main universe") -eq 0 ]]; then
          echo -e "deb http://archive.ubuntu.com/ubuntu ${repo_codename} main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      fi
    elif [[ "${repo_codename}" =~ (jessie|stretch|buster|bullseye|bookworm|trixie|forky) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
      ...
      if ! dpkg -l apt-transport-https 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
        ...
        if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install apt-transport-https &>> "${eus_dir}/logs/required.log"; then
          ...
          if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
            ...
            if [[ "${repo_codename}" =~ (disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then repo_arguments=" main universe"; fi
            if [[ "${archived_repo}" != "true" ]]; then
              if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}") -eq 0 ]]; then
                echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
              fi
            else
              if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}") -eq 0 ]]; then
                echo -e "deb http://archive.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
              fi
            fi
...
if ! dpkg -l software-properties-common 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install software-properties-common &>> "${eus_dir}/logs/required.log"; then
    ...
    if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      ...
      if [[ "${repo_codename}" =~ (trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then repo_arguments=" main"; fi
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
...
if ! dpkg -l curl 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install curl &>> "${eus_dir}/logs/required.log"; then
    ...
    if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      ...
      if [[ "${repo_codename}" =~ (disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then repo_arguments=" main"; fi
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
...
if ! dpkg -l dirmngr 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install dirmngr &>> "${eus_dir}/logs/required.log"; then
    ...
    if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename} universe") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename} universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename} main restricted") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename} main restricted" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      else
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu ${repo_codename} universe") -eq 0 ]]; then
          echo -e "deb http://archive.ubuntu.com/ubuntu ${repo_codename} universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu ${repo_codename} main restricted") -eq 0 ]]; then
          echo -e "deb http://archive.ubuntu.com/ubuntu ${repo_codename} main restricted" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      fi
    elif [[ "${repo_codename}" =~ (jessie|stretch|buster|bullseye|bookworm|trixie|forky) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
...
if ! dpkg -l psmisc 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install psmisc &>> "${eus_dir}/logs/required.log"; then
    ...
    if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      ...
      if [[ "${repo_codename}" =~ (trusty|xenial|bionic|cosmicdisco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then repo_arguments=" universe"; fi
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu ${repo_codename}${repo_arguments}" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
...
if [[ "${os_codename}" =~ (disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) && "${architecture}" =~ (amd64|arm64) ]]; then
  header
...
if ! dpkg -l | grep "^ii\\|^hi" | grep -iq "mongodb-server\\|mongodb-org-server"; then
  ...
  if [[ "${os_codename}" =~ (precise|maya|trusty|qiana|rebecca|rafaela|rosa|xenial|sarah|serena|sonya|sylvia) && ! "${architecture}" =~ (amd64|arm64) ]]; then
    ubuntu_32_mongo
    if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-server mongodb-clients; then
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu xenial main universe") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu xenial main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
          hide_apt_update=true
          run_apt_get_update
        fi
      else
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu xenial main universe") -eq 0 ]]; then
          echo -e "deb http://archive.ubuntu.com/ubuntu xenial main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
          hide_apt_update=true
          run_apt_get_update
        fi
      fi
      echo -e "${WHITE_R}#${RESET} Installing mongodb-server and mongodb-clients..."
	  if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-server mongodb-clients &>> "${eus_dir}/logs/mongodb_install.log"; then
        ...
        if apt-get install -f &>> "${eus_dir}/logs/mongodb_install.log"; then
          ...
          if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-server mongodb-clients &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully installed mongodb-server and mongodb-clients! \\n"; else abort; fi
        else
          echo -e "${RED}#${RESET} Failed to run apt-get install -f! \\n"; abort
        fi
      fi
    fi
  elif [[ "${os_codename}" =~ (bionic|tara|tessa|tina|tricia|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) && "${architecture}" == "i386" ]]; then
    ubuntu_32_mongo
    libssl_temp="$(mktemp --tmpdir=/tmp libssl1.0.2_XXXXX.deb)" || abort
    libssl_url=$(curl -s http://ftp.debian.org/debian/pool/main/o/openssl1.0/ | grep -io "libssl1.0.2.*i386.deb" | tail -n1)
    echo -e "${WHITE_R}#${RESET} Downloading libssl..."
    if wget "${wget_progress[@]}" -O "$libssl_temp" "http://ftp.debian.org/debian/pool/main/o/openssl1.0/${libssl_url}" &>> "${eus_dir}/logs/libssl.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded libssl!"; else echo -e "${RED}#${RESET} Failed to download libssl..."; abort; fi
    echo -e "\\n${WHITE_R}#${RESET} Installing libssl..."
    if dpkg -i "$libssl_temp" &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully installed libssl! \\n"; else echo -e "${RED}#${RESET} Failed to install libssl! \\n"; abort; fi
    rm --force "$libssl_temp" 2> /dev/null
    if [[ "${os_codename}" =~ (disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://old-releases.ubuntu.com/ubuntu bionic main universe") -eq 0 ]]; then
          echo -e "deb http://old-releases.ubuntu.com/ubuntu bionic main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
          hide_apt_update=true
          run_apt_get_update
        fi
      else
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.ubuntu.com/ubuntu bionic main universe") -eq 0 ]]; then
          echo -e "deb http://archive.ubuntu.com/ubuntu bionic main universe" >>/etc/apt/sources.list.d/glennr-install-script.list || abort
          hide_apt_update=true
          run_apt_get_update
        fi
      fi
    fi
    if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install libboost-chrono1.62.0 libboost-filesystem1.62.0 libboost-program-options1.62.0 libboost-regex1.62.0 libboost-system1.62.0 libboost-thread1.62.0 libgoogle-perftools4 libpcap0.8 libpcrecpp0v5 libsnappy1v5 libstemmer0d libyaml-cpp0.5v5 &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully installed required MongoDB packages! \\n"; else echo -e "${RED}#${RESET} Failed install required MongoDB packages... \\n"; abort; fi
    mongo_tools_url=$(curl -s http://ftp.debian.org/debian/pool/main/m/mongo-tools/ | grep -io "mongo-tools_3.2.*i386.deb" | sed 's/&#43;/+/g' | tail -n1)
    ...
    mongodb_clients_url=$(curl -s http://ftp.debian.org/debian/pool/main/m/mongodb/ | grep -io "mongodb-clients_3.2.*i386.deb" | sed 's/&#43;/+/g' | tail -n1)
    ...
    mongodb_server_url=$(curl -s http://ftp.debian.org/debian/pool/main/m/mongodb/ | grep -io "mongodb-server_3.2.*i386.deb" | sed 's/&#43;/+/g' | tail -n1)
    ...
    if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-org &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully installed mongodb-org version 3.4! \\n"; else echo -e "${RED}#${RESET} Failed to install mongodb-org version 3.4...\\n"; abort; fi
    ...
    run_apt_get_update
    echo -e "${WHITE_R}#${RESET} Installing mongodb-org version ${mongo_version_supported::3}..."
    if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-org &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully installed mongodb-org version ${mongo_version_supported::3}! \\n"; else echo -e "${RED}#${RESET} Failed to install mongodb-org version ${mongo_version_supported::3}...\\n"; abort; fi
  elif [[ "${os_codename}" =~ (xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|sarah|serena|sonya|sylvia|tara|tessa|tina|tricia) && "${architecture}" =~ (amd64|arm64) ]]; then
    if [[ "${mongo_version_supported}" == "3.6.999" ]]; then
      mongodb_36_key
    else
      mongodb_34_key
    ...
    if [[ "${os_codename}" =~ (jessie|stretch|continuum|buster|bullseye|bookworm|trixie|forky) && "${architecture}" =~ (amd64|arm64) ]]; then
      if [[ "${mongo_version_supported}" == "3.6.999" ]]; then
        mongodb_36_key
      else
        mongodb_34_key
...
if [[ "${first_digit_unifi}" -gt '7' ]] || [[ "${first_digit_unifi}" == '7' && "${second_digit_unifi}" -ge '4' ]] || [[ "${first_digit_unifi}" == '7' && "${second_digit_unifi}" -ge '5' ]]; then
  ...
  if [[ "${first_digit_unifi}" == '7' && "${second_digit_unifi}" -ge '4' ]]; then minimum_required_mongodb_version_dot="2.6"; minimum_required_mongodb_version="26"; unifi_latest_supported_version_number="7.3"; fi
...
if ! dpkg -l | grep "^ii\\|^hi" | grep -iq "${required_java_version}" || [[ "${old_openjdk_version}" == 'true' ]]; then
  ...
  if [[ "${repo_codename}" =~ (precise|trusty|xenial|bionic|cosmic) ]]; then
    ...
    if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${required_java_version}-jre-headless" &> /dev/null || [[ "${old_openjdk_version}" == 'true' ]]; then
  ...
  elif [[ "${repo_codename}" =~ (disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar) ]]; then
    echo -e "${WHITE_R}#${RESET} ${openjdk_variable} ${required_java_version}-jre-headless..."
    if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${required_java_version}-jre-headless" &> /dev/null || [[ "${old_openjdk_version}" == 'true' ]]; then
      echo -e "${RED}#${RESET} Failed to ${openjdk_variable_3} ${required_java_version}-jre-headless in the first run...\\n"
    ...
    echo -e "${WHITE_R}#${RESET} ${openjdk_variable} ${required_java_version}-jre-headless..."
    if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install -t jessie-backports "${required_java_version}-jre-headless" &> /dev/null || [[ "${old_openjdk_version}" == 'true' ]]; then
      echo -e "${RED}#${RESET} Failed to ${openjdk_variable_3} ${required_java_version}-jre-headless in the first run...\\n"
    ...
    if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${required_java_version}-jre-headless" &> /dev/null || [[ "${old_openjdk_version}" == 'true' ]]; then
      ...
      if [[ "${required_java_version}" == "openjdk-8" ]]; then
        if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://archive.debian.org/debian stretch main") -eq 0 ]]; then
          echo "deb http://archive.debian.org/debian stretch main" >> /etc/apt/sources.list.d/glennr-install-script.list || abort
        fi
      elif [[ "${required_java_version}" =~ (openjdk-11|openjdk-17) ]]; then
        if [[ "${repo_codename}" =~ (bookworm|trixie|forky) ]] && [[ "${required_java_version}" =~ (openjdk-11) ]]; then repo_codename="bullseye"; fi
        if [[ "${repo_codename}" =~ (buster|forky) ]] && [[ "${required_java_version}" =~ (openjdk-17) ]]; then repo_codename="bookworm"; fi
        if [[ "${archived_repo}" != "true" ]]; then
          if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb http[s]*://ftp.debian.org/debian ${repo_codename} main") -eq 0 ]]; then
            echo -e "deb http://ftp.debian.org/debian ${repo_codename} main" >> /etc/apt/sources.list.d/glennr-install-script.list || abort
          fi
...
if [[ "${os_codename}" =~ (precise|maya|trusty|qiana|rebecca|rafaela|rosa|xenial|sarah|serena|sonya|sylvia|bionic|tara|tessa|tina|tricia|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|stretch|continuum|buster|bullseye|bookworm|trixie|forky) ]]; then
